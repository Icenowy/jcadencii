...
public class RateConvertContext {
    ...
    /// <returns></returns>
    public static int estimateResultSamples(RateConvertContext context,
        int length) {
    ...
    /// <param name="length"></param>
    public static boolean convert(RateConvertContext context, double[] left,
        double[] right, int length) {
        ...
        if (context.aRate == context.bRate) {
            ...
            if ((context.aEnd + 1) < aNext) {
                context.bBufBase = (long) ((context.aEnd + 1) * context.invARate * context.bRate) -
                    2; // aEnd + 1番目のデータを作成するのに必要なデータ点のインデクス
                ...
                if (num > 0) {
                    ...
                    for (int j = 0; j < num; j++) {
                        int indx = (int) ((context.bBufBase + j) -
                            context.bCount);
