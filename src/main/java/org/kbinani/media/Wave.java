...
public class Wave {
    ...
    public Wave(WaveChannel channels, int bit_per_sample, int sample_rate,
    ...
    public void replace(Wave srcWave, int srcStart, int destStart, int count) {
        if ((m_channel != srcWave.m_channel) ||
                (m_bit_per_sample != srcWave.m_bit_per_sample)) {
            return;
        }
        ...
        if (m_channel == WaveChannel.Monoral) {
            ...
            if (m_bit_per_sample == 8) {
                if ((L8 == null) || (R8 == null) || (srcWave.L8 == null) ||
                        (srcWave.R8 == null)) {
                    return;
                }
            } else {
                if ((L16 == null) || (R16 == null) || (srcWave.L16 == null) ||
                        (srcWave.R16 == null)) {
                    return;
                }
            }
        ...
        count = (int) ((count > (srcWave.getTotalSamples() - srcStart))
            ? (srcWave.getTotalSamples() - srcStart) : count);
    ...
    public void replace(byte[] data, int start_index) {
        if ((m_channel != WaveChannel.Monoral) || (m_bit_per_sample != 8) ||
                (L8 == null)) {
            return;
        }
    ...
    public void replace(short[] data, int start_index) {
        if ((m_channel != WaveChannel.Monoral) || (m_bit_per_sample != 16) ||
                (L16 == null)) {
            return;
        }
    ...
    public void replace(byte[] left, byte[] right, int start_index) {
        if ((m_channel != WaveChannel.Stereo) || (m_bit_per_sample != 8) ||
                (L8 == null) || (R8 == null)) {
            return;
        }
    ...
    public void replace(short[] left, short[] right, int start_index) {
        if ((m_channel != WaveChannel.Stereo) || (m_bit_per_sample != 16) ||
                (L16 == null) || (R16 == null)) {
            return;
        }
    ...
    private static void writeByteArray(RandomAccessFile fs, byte[] dat,
    ...
    private static double make_double_from_extended(byte[] bytes) {
        ...
        hiMant = ((long) (bytes[2] & 0xFF) << 24) |
            ((long) (bytes[3] & 0xFF) << 16) | ((long) (bytes[4] & 0xFF) << 8) |
            ((long) (bytes[5] & 0xFF));
        loMant = ((long) (bytes[6] & 0xFF) << 24) |
            ((long) (bytes[7] & 0xFF) << 16) | ((long) (bytes[8] & 0xFF) << 8) |
            ((long) (bytes[9] & 0xFF));
    ...
    private boolean parseWaveHeader(RandomAccessFile fs) {
        try {
            ...
            if ((buf[0] != 0x57) || (buf[1] != 0x41) || (buf[2] != 0x56) ||
                    (buf[3] != 0x45)) {
                fs.close();
            ...
            if ((buf[0] != 0x66) || (buf[1] != 0x6d) || (buf[2] != 0x74) ||
                    (buf[3] != 0x20)) {
                fs.close();
    ...
    public boolean read(String path) {
        ...
        try {
            ...
            if ((buf[0] == 0x52) && (buf[1] == 0x49) && (buf[2] == 0x46) &&
                    (buf[3] == 0x46)) {
                ret = parseWaveHeader(fs);
            } else if ((buf[0] == 0x46) && (buf[1] == 0x4f) &&
    ...
    private static void copyArray(short[] src, int src_start, short[] dest,
        int dest_start, int length) {
        for (int i = 0; i < length; i++) {
            dest[i + dest_start] = src[i + src_start];
        }
    }

    private static void copyArray(byte[] src, int src_start, byte[] dest,
        int dest_start, int length) {
        for (int i = 0; i < length; i++) {
            dest[i + dest_start] = src[i + src_start];
        }
    }
    public enum Channel {Right,
        Left;
    }
    public enum WaveChannel {Monoral,
        Stereo;
    }
}
