...
public class EditedZone implements Cloneable {
    ...
    public EditedZoneCommand executeCommand(EditedZoneCommand run) {
        for (Iterator<EditedZoneUnit> itr = run.mRemove.iterator();
                itr.hasNext();) {
    ...
    private EditedZoneCommand generateCommandAdd(EditedZoneUnit[] areas) {
        ...
        for (Iterator<EditedZoneUnit> itrThis = iterator(); itrThis.hasNext();) {
            ...
            for (Iterator<EditedZoneUnit> itrWork = work.iterator();
                    itrWork.hasNext();) {
                EditedZoneUnit itemWork = itrWork.next();

                if ((itemThis.mStart == itemWork.mStart) &&
                        (itemThis.mEnd == itemWork.mEnd)) {
                    found = true;
        ...
        for (Iterator<EditedZoneUnit> itrWork = work.iterator();
            ...
            for (Iterator<EditedZoneUnit> itrThis = iterator();
                    itrThis.hasNext();) {
                EditedZoneUnit itemThis = itrThis.next();
    ...
    private EditedZoneCommand generateCommandAdd(int start, int end) {
        return generateCommandAdd(new EditedZoneUnit[] {
                new EditedZoneUnit(start, end)
            });
    }
        ...
        while (changed) {
            ...
            for (int i = 0; i < (count - 1); i++) {
                ...
                for (int j = i + 1; j < count; j++) {
                    ...
                    if ((itemj.mStart == itemi.mStart) &&
                            (itemj.mEnd == itemi.mEnd)) {
                        mSeries.removeElementAt(j);
                        changed = true;

                        break;
                    } else if ((itemj.mStart <= itemi.mStart) &&
                            (itemi.mEnd <= itemj.mEnd)) {
                        mSeries.removeElementAt(i);
                        changed = true;

                        break;
                    } else if ((itemi.mStart <= itemj.mStart) &&
                            (itemj.mEnd <= itemi.mEnd)) {
                        mSeries.removeElementAt(j);
                        changed = true;

                        break;
                    } else if ((itemi.mStart <= itemj.mEnd) &&
                            (itemj.mEnd < itemi.mEnd)) {
                        itemj.mEnd = itemi.mEnd;
                        mSeries.removeElementAt(i);
                        changed = true;
