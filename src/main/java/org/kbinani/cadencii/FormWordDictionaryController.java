...
public class FormWordDictionaryController extends ControllerBase
    implements FormWordDictionaryUiListener {
    ...
    public void buttonDownClick() {
        ...
        if ((0 <= index) &&
                ((index + 1) < ui.listDictionariesGetItemCountRow())) {
            try {
    ...
    public Vector<ValuePair<String, Boolean>> getResult() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ret.add(new ValuePair<String, Boolean>(ui.listDictionariesGetItemAt(
                        i), ui.listDictionariesIsRowChecked(i)));
        }
