...
public class EditorConfig {
    ...
    private static String getLastUsedPathCore(Vector<String> list,
        String extension) {
        if (extension == null) {
            return "";
        }

        if (PortUtil.getStringLength(extension) <= 0) {
            return "";
        }
    ...
    /// <returns></returns>
    public VibratoHandle createAutoVibrato(SynthesizerType type,
        int vibrato_clocks) {
        if (UseUserDefinedAutoVibratoType) {
            ...
            if (this.AutoVibratoTypeCustom == null) {
                ...
                if (trimlen > 0) {
                    String s = str.sub(this.AutoVibratoTypeCustom,
                            len - trimlen, trimlen);
                    ...
                    try {
                        ...
                        serr.println(EditorConfig.class +
                            ".createAutoVibrato; ex=" + ex +
                            "; AutoVibratoTypeCustom=" + AutoVibratoTypeCustom +
                            "; s=" + s);
                        index = 0;
                    }
                }
            ...
            String iconid = (type == SynthesizerType.VOCALOID1)
                ? AutoVibratoType1 : AutoVibratoType2;
    ...
    public TreeMap<String, BKeys[]> getShortcutKeysDictionary(
        Vector<ValuePairOfStringArrayOfKeys> defs) {
        ...
        for (Iterator<ValuePairOfStringArrayOfKeys> itr = defs.iterator();
                itr.hasNext();) {
    ...
    public void setPositionQuantize(QuantizeMode value) {
        if (m_position_quantize != value) {
            ...
            try {
                ...
                Logger.write(EditorConfig.class + ".getPositionQuantize; ex=" +
                    ex + "\n");
    ...
    public void setPositionQuantizeTriplet(boolean value) {
        if (m_position_quantize_triplet != value) {
            ...
            try {
                ...
                serr.println("EditorConfig#setPositionQuantizeTriplet; ex=" +
                    ex);
                Logger.write(EditorConfig.class +
                    ".setPositionQuantizeTriplet; ex=" + ex + "\n");
            }
        }
    }
    ...
    public void setLengthQuantize(QuantizeMode value) {
        if (m_length_quantize != value) {
            ...
            try {
                ...
                Logger.write(EditorConfig.class + ".setLengthQuantize; ex=" +
                    ex + "\n");
            }
        }
    }
    ...
    public void setLengthQuantizeTriplet(boolean value) {
        if (m_length_quantize_triplet != value) {
            ...
            try {
                ...
                Logger.write(EditorConfig.class +
                    ".setLengthQuantizeTriplet; ex=" + ex + "\n");
            }
        }
    }
    ...
    public void check() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (Iterator<String> itr = UserDictionaries.iterator();
                    itr.hasNext();) {
                String s = itr.next();
        ...
        if (vec.size(PathResamplers) != vec.size(ResamplersWithWine)) {
            int delta = vec.size(ResamplersWithWine) -
                vec.size(PathResamplers);

            if (delta > 0) {
                for (int i = 0; i < delta; i++) {
                    ResamplersWithWine.removeElementAt(vec.size(
                            ResamplersWithWine) - 1);
                }
            } else if (delta < 0) {
                for (int i = 0; i < -delta; i++) {
