...
public abstract class WaveUnit {
    ...
    /// この値を使うかどうかは任意
    /// </summary>
    public static final int BASE_HEIGHT_PER_PORTS = 20;
    ...
    public void setRoot(WaveGenerator root) {
        mRoot = root;
    }
    ...
    /// メインウィンドウへの参照を設定します
    /// </summary>
    public void setMainWindow(FormMain main_window) {
    ...
    public void paintTo(Graphics2D graphics, int x, int y, int width, int height) {
        ...
        // 枠と背景を描画
        paintBackground(graphics, mStroke, x, y, width, height, Color.black,
            PortUtil.Pink);

        // デバイス名を書く
        String typename = this.getClass().getSimpleName();
        PortUtil.drawStringEx((Graphics) graphics, typename, mFont,
            new Rectangle(x, y, width, height), PortUtil.STRING_ALIGN_CENTER,
    ...
    protected void paintBackground(Graphics2D graphics, Stroke stroke, int x,
        int y, int width, int height, Color border, Color background) {
        // 背景を塗りつぶし
