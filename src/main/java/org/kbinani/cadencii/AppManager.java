...
// idsに登録された音符のうち、前後がつながっているものを列挙する。
     ...
     * 波形生成器を別スレッドで実行します
     *
     * @param samples 合成するサンプル数．波形合成器のbeginメソッドに渡される
     */
     ...
     *
     * @param kind 音声合成システムの種類
     * @return 音声合成システムを識別する文字列(VOCALOID2=DSB301, VOCALOID1[1.0, 1.1]=DSB202, AquesTone=AQT000, Straight x UTAU=STR000, UTAU=UTAU000)
     */
...
// 戻りのテキストファイルを読み込む
        ...
        PlaySound.init();
        mLocker = new Object();
        //Set look and feel theme for each platform,so it will look more like native apps
        String LF = null;
        String SystemPlat = System.getProperty("os.name");
        if (SystemPlat.equals("Windows")) {
            LF = "com.sun.java.swing.plaf.windows.WindowsLookAndFeel";
        } else if (SystemPlat.equals("Linux")) {
            LF = "com.sun.java.swing.plaf.gtk.GTKLookAndFeel";
        } else if (SystemPlat.equals("Macintosh")) {
...
// 日本語辞書
        SymbolTable.loadDictionary(
                PointD.class.getResourceAsStream("/assets/dict_ja.txt"),
                "DEFAULT_JP");
// 英語辞書
        SymbolTable.loadDictionary(
                PointD.class.getResourceAsStream("/assets/dict_en.txt"),
                "DEFAULT_EN");
// 中国語辞書
        SymbolTable.loadDictionary(
                PointD.class.getResourceAsStream("/assets/dict_cn.txt"),
                "DEFAULT_CN");
// 拡張辞書
        SymbolTable.loadAllDictionaries("udic");
