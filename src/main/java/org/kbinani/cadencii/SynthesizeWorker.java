...
public class SynthesizeWorker {
    ...
    public void patchWork(WorkerState state, Object arg) {
        ...
        for (int k = 0; k < tracks.size(); k++) {
            ...
            if ((queueIndex.size() == 1) &&
                    wavePath.equals(queue.get(queueIndex.get(0)).file)) {
                // 第trackトラック全体の合成を指示するキューだった場合．
                // このとき，パッチワークを行う必要なし．
                AppManager.mLastRenderedStatus[track - 1] = new RenderedStatus((VsqTrack) vsq.Track.get(
                            track).clone(), vsq.TempoTable,
                        (SequenceConfig) vsq.config.clone());
                AppManager.serializeRenderingStatus(temppath, track);
                AppManager.invokeWaveViewReloadRequiredEvent(track, wavePath,
                    1, -1);
            ...
            try {
                ...
                long totalLength = (long) ((vsq.getSecFromClock(vsq.TotalClocks) +
                    1.0) * sampleRate);
                writer = new WaveWriter(wavePath,
                        vsq.config.WaveFileOutputChannel, 16, sampleRate);
                ...
                for (int m = 0; m < queueIndex.size(); m++) {
                    ...
                    try {
                        ...
                        while (remain2 > 0) {
                            int delta = (remain2 > BUFLEN) ? BUFLEN
                                                           : (int) remain2;
                        ...
                        Logger.write(AppManager.class +
                            ".patchWorkToFreeze; ex=" + ex + "\n");
                        ...
                        if (wr != null) {
                            try {
                                ...
                                Logger.write(AppManager.class +
                                    ".patchWorkToFreeze; ex=" + ex2 + "\n");
                                serr.println(
                                    "AppManager#patchWorkToFreeze; ex2=" + ex2);
                                ex2.printStackTrace();
                            }
                        }
                    }
                ...
                if (queueIndex.get(queueIndex.size() - 1) <= finished) {
                    ...
                    AppManager.mLastRenderedStatus[track - 1] = new RenderedStatus((VsqTrack) vsq_track.clone(),
                            vsq.TempoTable, (SequenceConfig) vsq.config.clone());
                    AppManager.serializeRenderingStatus(temppath, track);
                    ...
                    for (int m = 0; m < queueIndex.size(); m++) {
                        ...
                        for (Iterator<Integer> itr = copied.indexIterator(
                                    IndexIteratorKind.SINGER); itr.hasNext();) {
                            int j = itr.next();
                    ...
                    AppManager.mLastRenderedStatus[track - 1] = new RenderedStatus(copied,
                            vsq.TempoTable, (SequenceConfig) vsq.config.clone());
                    AppManager.serializeRenderingStatus(temppath, track);
                }
                ...
                if (writer != null) {
                    try {
                        ...
                        Logger.write(AppManager.class +
                            ".patchWorkToFreeze; ex=" + ex2 + "\n");
                        serr.println("AppManager#patchWorkToFreeze; ex2=" +
                            ex2);
                        ex2.printStackTrace();
                    }
                }
    ...
    public void processQueue(WorkerState state, Object arg) {
        ...
        double amp_track = VocaloSysUtil.getAmplifyCoeffFromFeder(vsq.Mixer.Slave.get(track -
                    1).Feder);
        double pan_left_track = VocaloSysUtil.getAmplifyCoeffFromPanLeft(vsq.Mixer.Slave.get(track -
                    1).Panpot);
        double pan_right_track = VocaloSysUtil.getAmplifyCoeffFromPanRight(vsq.Mixer.Slave.get(track -
                    1).Panpot);
            ...
            if (numTrack > 2) {
                for (int i = 1; i < numTrack; i++) {
                    ...
                    try {
                        ...
                        Logger.write(SynthesizeWorker.class +
                            ".processQueue; ex=" + ex + "\n");
                        r = null;
                    }
                    ...
                    double amp_i = VocaloSysUtil.getAmplifyCoeffFromFeder(vsq.Mixer.Slave.get(i -
                                1).Feder);
                    double pan_left_i = VocaloSysUtil.getAmplifyCoeffFromPanLeft(vsq.Mixer.Slave.get(i -
                                1).Panpot);
                    double pan_right_i = VocaloSysUtil.getAmplifyCoeffFromPanRight(vsq.Mixer.Slave.get(i -
                                1).Panpot);
        ...
        FileWaveReceiver wave_receiver = new FileWaveReceiver(q.file, channel,
                16, sample_rate);
