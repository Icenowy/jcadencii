...
public class VersionInfo extends BDialog {
    ...
    private BufferedImage generateAuthorListB(boolean show_twitter_id) {
        ...
        Dimension size = Util.measureString("the quick brown fox jumped over the lazy dogs. THE QUICK BROWN FOX JUMPED OVER THE LAZY DOGS. 0123456789",
                font);
        int width = getWidth();
        int height = size.height;

        //StringFormat sf = new StringFormat();
        BufferedImage ret = new BufferedImage((int) width,
                (int) (40f + (m_credit.length * height * 1.1f)),
        ...
        Font f = new Font(font_name, java.awt.Font.BOLD,
                (int) (FONT_SIZE * 1.2f));

        if (m_shadow_enablde) {
            g.setColor(new Color(0, 0, 0, 40));
            PortUtil.drawStringEx(g, m_app_name, f,
        ...
        PortUtil.drawStringEx(g, m_app_name, f,
            new Rectangle(0, height, width, height), align, valign);
        ...
        for (int i = 0; i < m_credit.length; i++) {
            ...
            String str = itemi.getName() +
                (id.equals("") ? "" : (" (" + id + ")"));

            if (m_shadow_enablde) {
                g.setColor(new Color(0, 0, 0, 40));
                PortUtil.drawStringEx(g, str, font,
                    new Rectangle(0 + shadow_shift,
                        40 + (int) (i * height * 1.1) + shadow_shift + height,
                        width, height), align, valign);
            }

            g.setColor(Color.black);
            PortUtil.drawStringEx(g, str, f2,
                new Rectangle(0, 40 + (int) (i * height * 1.1) + height, width,
                    height), align, valign);
        }
    ...
    public void btnFlip_Click(Object sender, BEventArgs e) {
        ...
        if (m_credit_mode) {
            try {
                btnFlip.setText(PortUtil.formatMessage(gettext("About {0}"),
                        m_app_name));
            } catch (Exception ex) {
    ...
    private void paintCor(Graphics g1) {
        ...
        if (m_credit_mode) {
            float times = (float) (PortUtil.getCurrentTime() -
                m_scroll_started) - 3f;
            ...
            Rectangle bottom = new Rectangle(0, m_height - grad_height,
                    getWidth(), grad_height);
            g.clipRect(0, m_height - grad_height + 1, getWidth(),
                grad_height - 1);
            g.setClip(null);
        } else {
            g.setFont(new Font("Century Gorhic", java.awt.Font.BOLD,
                    FONT_SIZE * 2));
    ...
    private void registerEventHandlers() {
        this.panelCredit.paintEvent.add(new BPaintEventHandler(this,
                "VersionInfo_Paint"));
        ...
        this.chkTwitterID.checkedChangedEvent.add(new BEventHandler(this,
                "chkTwitterID_CheckedChanged"));
    }
