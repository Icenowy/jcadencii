...
public class VsqMetaText implements Cloneable, Serializable {
    ...
    private VsqMetaText(String name, int pre_measure, String singer,
        boolean is_first_track) {
    ...
    public VsqMetaText(TextStream sr) {
        ...
        while (true) {
            ...
            if (str.compare(last_line.value, "[Common]")) {
                ...
                while (!str.startsWith(last_line.value, "[")) {
                    String[] spl2 = PortUtil.splitString(last_line.value,
                            new char[] { '=' });
                    ...
                    if (!str.compare(spl2[1], "EOS")) {
                        ...
                        for (int i = 0; i < ids.length; i++) {
                            String[] spl3 = PortUtil.splitString(ids[i],
                                    new char[] { '#' });
                            id_number = str.toi(spl3[1]);
                            vec.add(t_event_list,
                                new ValuePair<Integer, Integer>(clock, id_number));
                        }
                    } else {
                        vec.add(t_event_list,
                            new ValuePair<Integer, Integer>(clock, -1));
                    }
        ...
        for (int i = 0; i < c; i++) {
            ...
            if (__handle.containsKey(id.IconHandle_index)) {
                if (id.type == VsqIDType.Singer) {
                    id.IconHandle = __handle.get(id.IconHandle_index)
                                            .castToIconHandle();
                } else if (id.type == VsqIDType.Aicon) {
                    id.IconDynamicsHandle = __handle.get(id.IconHandle_index)
                                                    .castToIconDynamicsHandle();
                }
            }

            if (__handle.containsKey(id.LyricHandle_index)) {
                id.LyricHandle = __handle.get(id.LyricHandle_index)
                                         .castToLyricHandle();
            }

            if (__handle.containsKey(id.VibratoHandle_index)) {
                id.VibratoHandle = __handle.get(id.VibratoHandle_index)
                                           .castToVibratoHandle();
            }

            if (__handle.containsKey(id.NoteHeadHandle_index)) {
                id.NoteHeadHandle = __handle.get(id.NoteHeadHandle_index)
                                            .castToNoteHeadHandle();
            }
        }
        ...
        for (int i = 0; i < vec.size(t_event_list); i++) {
            ...
            if (__id.containsKey(id_number)) {
                count++;
                Events.add(new VsqEvent(clock,
                        (VsqID) __id.get(id_number).clone()), count);
            }
        }
    ...
    public VsqBPList getElement(String curve) {
        ...
        String search = curve.trim().toLowerCase();
    ...
    private Vector<VsqHandle> buildHandleList() {
        ...
        for (Iterator<VsqEvent> itr = Events.iterator(); itr.hasNext();) {
            ...
            if (item.ID.IconHandle != null) {
                if (item.ID.IconHandle instanceof IconHandle) {
    ...
    private Vector<VsqHandle> writeEventListCor(ITextWriter writer, int eos)
        ...
        while (i < vec.size(temp)) {
            ...
            if (!item.ID.equals(VsqID.EOS)) {
                String ids = "ID#" +
                    PortUtil.formatDecimal("0000", item.ID.value);
                int clock = vec.get(temp, i).Clock;

                while (((i + 1) < vec.size(temp)) &&
                        (clock == vec.get(temp, i + 1).Clock)) {
                    i++;
                    ids += (",ID#" +
                    PortUtil.formatDecimal("0000", vec.get(temp, i + 1).ID.value));
                }
    ...
    public Vector<VsqHandle> writeEventList(BufferedWriter stream_writer,
        int eos) throws IOException {
