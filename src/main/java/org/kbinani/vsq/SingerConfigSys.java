...
public class SingerConfigSys {
    public static final int MAX_SINGERS = 0x4000;
    ...
    public SingerConfigSys(String path_voicedb, String[] path_installed_singers) {
        ...
        try {
            ...
            for (int language = 0; language < 0x80; language++) {
                for (int program = 0; program < 0x80; program++) {
                    ...
                    if (value >= 1) {
                        String vvd = fsys.combine(path_voicedb,
                                "vvoice" + value + ".vvd");
                        SingerConfig item = SingerConfig.fromVvd(vvd, language,
                                program);
        ...
        for (Iterator<SingerConfig> itr = m_installed_singers.iterator();
                itr.hasNext();) {
            SingerConfig sc = itr.next();
            String searchid = sc.VOICEIDSTR;

            for (Iterator<SingerConfig> itr2 = m_singer_configs.iterator();
                    itr2.hasNext();) {
    ...
    public VsqID getSingerID(int language, int program) {
        ...
        ret.IconHandle.IconID = "$0701" + PortUtil.toHexString(sc.Language, 2) +
            PortUtil.toHexString(sc.Program, 2);
