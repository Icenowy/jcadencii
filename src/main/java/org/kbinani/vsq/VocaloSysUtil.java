...
public class VocaloSysUtil {
    private static TreeMap<SynthesizerType, SingerConfigSys> s_singer_config_sys =
        new TreeMap<SynthesizerType, SingerConfigSys>();
    private static TreeMap<SynthesizerType, ExpressionConfigSys> s_exp_config_sys =
        new TreeMap<SynthesizerType, ExpressionConfigSys>();
    ...
    public static int getDefaultDseVersion() {
        if (!isInitialized) {
            serr.println(
                "VocaloSysUtil#getDefaultDseVersion; not initialized yet");
    ...
    public static String combineWinePath(String wine_prefix, String full_path) {
        ...
        String inner_path = (full_path_len >= 3)
            ? str.sub(full_path, 2).replace("\\", "/") : "";

        return fsys.combine(fsys.combine(wine_prefix, "drive_" + drive),
            inner_path);
    }
    ...
    public static void init(Vector<String> reg_list, String wine_prefix) {
        ...
        try {
            ...
            initExtract(dir1, header1, path_vsti, path_voicedb1, path_expdb1,
                path_editor, installed_singers1);
            ...
            if (str.length(wine_prefix) > 0) {
                for (int i = 0; i < act_installed_singers1.length; i++) {
                    act_installed_singers1[i] = combineWinePath(wine_prefix,
                            act_installed_singers1[i]);
                }

                act_path_voicedb1 = combineWinePath(wine_prefix,
                        act_path_voicedb1);
            ...
            String expression_map1 = fsys.combine(act_path_expdb1,
                    "expression.map");
            SingerConfigSys singer_config_sys = new SingerConfigSys(act_path_voicedb1,
                    act_installed_singers1);

            if (fsys.isFileExists(expression_map1)) {
                exp_config_sys1 = new ExpressionConfigSys(act_path_editor1,
                        act_path_expdb1);
            }
                ...
                if (fsys.isFileExists(ini)) {
                    ...
                    try {
                        br = new BufferedReader(new InputStreamReader(
                                    new FileInputStream(ini), "Shift_JIS"));
                            ...
                            if (line.startsWith("DSEVersion")) {
                                ...
                                if (spl.length >= 2) {
                                    ...
                                    try {
                                        ...
                                        serr.println("VocaloSysUtil#init; ex=" +
                                            ex);
                                        ex.printStackTrace();
            ...
            SingerConfigSys singer_config_sys = new SingerConfigSys("",
                    new String[] {  });
        ...
        try {
            ...
            ByRef<String> path_vsti = new ByRef<String>("");
            ByRef<String> path_editor = new ByRef<String>("");
            initExtract(dir2, header2, path_vsti, path_voicedb2, path_expdb2,
                path_editor, installed_singers2);
            ...
            if (str.length(wine_prefix) > 0) {
                for (int i = 0; i < act_installed_singers2.length; i++) {
                    act_installed_singers2[i] = combineWinePath(wine_prefix,
                            act_installed_singers2[i]);
                }

                act_path_expdb2 = combineWinePath(wine_prefix, act_path_expdb2);
                act_path_voicedb2 = combineWinePath(wine_prefix,
                        act_path_voicedb2);
                act_path_editor2 = combineWinePath(wine_prefix, act_path_editor2);
                act_vsti2 = combineWinePath(wine_prefix, act_vsti2);
            }

            String expression_map2 = fsys.combine(act_path_expdb2,
                    "expression.map");
            SingerConfigSys singer_config_sys = new SingerConfigSys(act_path_voicedb2,
                    act_installed_singers2);

            if (fsys.isFileExists(expression_map2)) {
                exp_config_sys2 = new ExpressionConfigSys(act_path_editor2,
                        act_path_expdb2);
            }
    ...
    /// <returns></returns>
    public static VibratoHandle getDefaultVibratoHandle(String icon_id,
        int vibrato_length, SynthesizerType type) {
        if (!isInitialized) {
            serr.println(
                "VocaloSysUtil#getDefaultVibratoHandle; not initialized yet");

            return null;
        }

        if (s_exp_config_sys.containsKey(type)) {
            for (Iterator<VibratoHandle> itr = s_exp_config_sys.get(type)
                                                               .vibratoConfigIterator();
                    itr.hasNext();) {
    ...
    private static void initExtract(Vector<String> dir, String header,
        ByRef<String> path_vsti, ByRef<String> path_voicedb,
        ByRef<String> path_expdb, ByRef<String> path_editor,
        ...
        for (Iterator<String> itr = dir.iterator(); itr.hasNext();) {
            ...
            if (s.startsWith(header + "\\APPLICATION")) {
                application.add(str.sub(s,
                        PortUtil.getStringLength(header + "\\APPLICATION")));
            } else if (s.startsWith(header + "\\DATABASE\\EXPRESSION")) {
                expression.add(str.sub(s,
                        PortUtil.getStringLength(header +
                            "\\DATABASE\\EXPRESSION")));
        ...
        for (Iterator<String> itr = install_dirs.keySet().iterator();
                itr.hasNext();) {
            String id = itr.next();
    ...
    private static void initPrint(String reg_key_name, String parent_name,
        Vector<String> list) {
    }
    ...
    /// <param name="type"></param>
    /// <returns></returns>
    public static Iterator<NoteHeadHandle> attackConfigIterator(
        SynthesizerType type) {
        if (!isInitialized) {
            serr.println(
                "VocaloSysUtil#attackConfigIterator; not initialized yet");
    ...
    /// <param name="type"></param>
    /// <returns></returns>
    public static Iterator<VibratoHandle> vibratoConfigIterator(
        SynthesizerType type) {
        if (!isInitialized) {
            serr.println(
                "VocaloSysUtil#vibratoConfigIterator; not initialized yet");
    ...
    /// <param name="type"></param>
    /// <returns></returns>
    public static Iterator<IconDynamicsHandle> dynamicsConfigIterator(
        SynthesizerType type) {
        if (!isInitialized) {
            serr.println(
                "VocaloSysUtil#dynamicsConfigIterator; not initialized yet");
    ...
    /// <param name="type"></param>
    /// <returns></returns>
    public static String getOriginalSinger(int language, int program,
        SynthesizerType type) {
        if (!isInitialized) {
        ...
        for (int i = 0; i < singer_configs.length; i++) {
            if ((language == singer_configs[i].Language) &&
                    (program == singer_configs[i].Program)) {
    ...
    /// <param name="type"></param>
    /// <returns></returns>
    public static VsqID getSingerID(int language, int program,
        SynthesizerType type) {
        if (!isInitialized) {
    ...
    public static VsqVoiceLanguage getLanguageFromName(String name) {
        if (!isInitialized) {
            serr.println(
                "VocaloSysUtil#getLanguageFromName; not initialized yet");
        ...
        if (search.equals("meiko") || search.equals("kaito") ||
                search.equals("miku") || search.equals("rin") ||
                search.equals("len") || search.equals("rin_act2") ||
                search.equals("len_act2") || search.equals("gackpoid") ||
                search.equals("luka_jpn") || search.equals("megpoid") ||
                search.equals("sfa2_miki") || search.equals("yuki") ||
                search.equals("kiyoteru") || search.equals("miku_sweet") ||
                search.equals("miku_dark") || search.equals("miku_soft") ||
                search.equals("miku_light") || search.equals("miku_vivid") ||
                search.equals("miku_solid")) {
            return VsqVoiceLanguage.Japanese;
        } else if (search.equals("sweet_ann") || search.equals("prima") ||
                search.equals("luka_eng") || search.equals("sonika") ||
                search.equals("lola") || search.equals("leon") ||
                search.equals("miriam") || search.equals("big_al")) {
