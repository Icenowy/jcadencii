...
public class BezierChain implements Cloneable, Serializable {
    ...
    /// <param name="x"></param>
    /// <returns></returns>
    public static PointD[] cutUnitBezier(PointD X0, PointD C0, PointD C1,
        PointD X1, double x) {
    ...
    public BezierChain extractPartialBezier(double t_start, double t_end)
        ...
        if ((start > t_start) || (t_end > end)) {
            throw new Exception(
                "no bezier point appeared in the range of \"from\" to \"to\"");
        }
        ...
        for (int i = 0; i < (this.points.size() - 1); i++) {
            if ((this.points.get(i).getBase().getX() < t_start) &&
                    (t_start < this.points.get(i + 1).getBase().getX())) {
                if ((this.points.get(i).getBase().getX() < t_end) &&
                        (t_end < this.points.get(i + 1).getBase().getX())) {
                    ...
                    left.setControlRightType(this.points.get(i)
                                                        .getControlRightType());
                    right.setControlLeftType(this.points.get(i + 1)
                                                        .getControlLeftType());
                    edited.add(left);
                    edited.add(right);
                    t_start_added = true;
                    ...
                    right.setControlRight(this.points.get(i + 1)
                                                     .getControlRight());
                    right.setControlRightType(this.points.get(i + 1)
                                                         .getControlRightType());
                    right.setControlLeftType(this.points.get(i + 1)
                                                        .getControlLeftType());
            ...
            if ((t_start <= this.points.get(i).getBase().getX()) &&
                    (this.points.get(i).getBase().getX() <= t_end)) {
                if (!t_start_added) {
        ...
        if ((this.points.get(this.points.size() - 1).getBase().getX() == t_end) &&
                !t_start_added) {
            edited.add((BezierPoint) this.points.get(this.points.size() - 1)
                                                .clone());
            count++;
        }
    ...
    public static boolean isBezierImplicit(BezierChain chain) {
        ...
        for (int i = 1; i < size; i++) {
            ...
            double pt2 = (last_point.getControlRightType() == BezierControlType.None)
                ? pt1 : last_point.getControlRight().getX();
            double pt4 = point.getBase().getX();
            double pt3 = (point.getControlLeftType() == BezierControlType.None)
                ? pt4 : point.getControlLeft().getX();
    ...
    /// <returns></returns>
    private static boolean isUnitBezierImplicit(double pt1, double pt2,
    ...
    /// <returns></returns>
    private static double solveCubicEquation(double a3, double a2, double a1,
        double a0, double ans) {
