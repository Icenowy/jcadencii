...
public class FormBezierPointEdit extends BDialog {
    ...
    public FormBezierPointEdit(TrackSelector parent, CurveType curve_type,
        int selected_chain_id, int selected_point_id) {
        super();
        initialize();
        ...
        for (Iterator<BezierPoint> itr = attached.getBezierChain(m_curve_type,
                    m_chain_id).points.iterator(); itr.hasNext();) {
            BezierPoint bp = itr.next();

            if (bp.getID() == m_point_id) {
                m_point = bp;
                smooth = (bp.getControlLeftType() != BezierControlType.None) ||
    ...
    private void updateStatus() {
        ...
        txtLeftClock.setText(((int) (m_point.getBase().getX() +
            m_point.controlLeft.getX())) + "");
        txtLeftValue.setText(((int) (m_point.getBase().getY() +
            m_point.controlLeft.getY())) + "");
        txtRightClock.setText(((int) (m_point.getBase().getX() +
            m_point.controlRight.getX())) + "");
        txtRightValue.setText(((int) (m_point.getBase().getY() +
            m_point.controlRight.getY())) + "");

        boolean smooth = (m_point.getControlLeftType() != BezierControlType.None) ||
    ...
    private void registerEventHandlers() {
        ...
        chkEnableSmooth.checkedChangedEvent.add(new BEventHandler(this,
                "chkEnableSmooth_CheckedChanged"));
        btnLeft.mouseMoveEvent.add(new BMouseEventHandler(this,
                "common_MouseMove"));
        btnLeft.mouseDownEvent.add(new BMouseEventHandler(this,
                "handleOperationButtonMouseDown"));
        btnLeft.mouseUpEvent.add(new BMouseEventHandler(this, "common_MouseUp"));
        btnDataPoint.mouseMoveEvent.add(new BMouseEventHandler(this,
                "common_MouseMove"));
        btnDataPoint.mouseDownEvent.add(new BMouseEventHandler(this,
                "handleOperationButtonMouseDown"));
        btnDataPoint.mouseUpEvent.add(new BMouseEventHandler(this,
                "common_MouseUp"));
        btnRight.mouseMoveEvent.add(new BMouseEventHandler(this,
                "common_MouseMove"));
        btnRight.mouseDownEvent.add(new BMouseEventHandler(this,
                "handleOperationButtonMouseDown"));
        btnRight.mouseUpEvent.add(new BMouseEventHandler(this, "common_MouseUp"));
        btnBackward.clickEvent.add(new BEventHandler(this,
                "handleMoveButtonClick"));
        btnForward.clickEvent.add(new BEventHandler(this,
                "handleMoveButtonClick"));
    }
    ...
    public void btnOK_Click(Object sender, BEventArgs e) {
        try {
            ...
            if ((y < m_min) || (m_max < y)) {
                AppManager.showMessageBox(gettext("Invalid value"),
                    gettext("Error"),
            ...
            AppManager.showMessageBox(gettext("Integer format error"),
                gettext("Error"),
                org.kbinani.windows.forms.Utility.MSGBOX_DEFAULT_OPTION,
                org.kbinani.windows.forms.Utility.MSGBOX_ERROR_MESSAGE);
    ...
    public void chkEnableSmooth_CheckedChanged(Object sender, BEventArgs e) {
        ...
        boolean old = (m_point.getControlLeftType() != BezierControlType.None) ||
            (m_point.getControlRightType() != BezierControlType.None);
    ...
    public void handleOperationButtonMouseDown(Object sender, BMouseEventArgs e) {
        ...
        Point loc_on_trackselector = new Point(AppManager.xCoordFromClocks(
                    (int) pd.getX()), m_parent.yCoordFromValue((int) pd.getY()));
        Point loc_topleft = m_parent.getLocationOnScreen();
        mScreenMouseDownLocation = new Point(loc_topleft.x +
                loc_on_trackselector.x, loc_topleft.y + loc_on_trackselector.y);
        PortUtil.setMousePosition(mScreenMouseDownLocation);

        BMouseEventArgs event_arg = new BMouseEventArgs(BMouseButtons.Left, 0,
    ...
    public void common_MouseUp(Object sender, BMouseEventArgs e) {
        ...
        Point loc_on_trackselector = new Point(loc_on_screen.x -
                loc_trackselector.x, loc_on_screen.y - loc_trackselector.y);
        BMouseEventArgs event_arg = new BMouseEventArgs(BMouseButtons.Left, 0,
                loc_on_trackselector.x, loc_on_trackselector.y, 0);
    ...
    public void common_MouseMove(Object sender, BMouseEventArgs e) {
        if (m_btn_datapoint_downed) {
            ...
            Point loc_on_trackselector = new Point(loc_on_screen.x -
                    loc_trackselector.x, loc_on_screen.y - loc_trackselector.y);
            BMouseEventArgs event_arg = new BMouseEventArgs(BMouseButtons.Left,
                    0, loc_on_trackselector.x, loc_on_trackselector.y, 0);
            BezierPoint ret = m_parent.HandleMouseMoveForBezierMove(event_arg,
                    m_picked_side);
    ...
    public void handleMoveButtonClick(Object sender, BEventArgs e) {
        ...
        BezierChain target = AppManager.getVsqFile().AttachedCurves.get(m_track -
                1).getBezierChain(m_curve_type, m_chain_id);
        int index = -2;
    ...
    private BButton getBtnLeft() {
        if (btnLeft == null) {
            btnLeft = new BButton();
            btnLeft.setText("");
            btnLeft.setIcon(new ImageIcon(getClass()
                                              .getResource("/target--pencil.png")));
        }
    ...
    private BButton getBtnDataPoint() {
        if (btnDataPoint == null) {
            ...
            btnDataPoint.setIcon(new ImageIcon(getClass()
                                                   .getResource("/target--pencil.png")));
        }
    ...
    private BButton getBtnRight() {
        if (btnRight == null) {
            ...
            btnRight.setText("");
            btnRight.setIcon(new ImageIcon(getClass()
                                               .getResource("/target--pencil.png")));
        }
