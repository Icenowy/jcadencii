...
class WaveformZoomController extends ControllerBase
    implements WaveformZoomUiListener {
    /// <summary>
    ...
    public void receivePaintSignal(Graphics g) {
        ...
        g.setColor(mWaveViewButtonAutoMaximizeMouseDowned ? PortUtil.Gray
                                                          : PortUtil.LightGray);
        ...
        g.drawString("Auto Maximize", SPACE + 16 + SPACE,
            (SPACE + (AppManager.superFont8Height / 2)) -
        ...
        if (!mWaveViewAutoMaximize) {
            g.setColor(mWaveViewButtonZoomMouseDowned ? PortUtil.Gray
                                                      : PortUtil.LightGray);
        ...
        PortUtil.drawStringEx(g,
            (mWaveViewButtonZoomMouseDowned ? "↑Move Mouse↓" : "Zoom"),
            AppManager.superFont9, rc, PortUtil.STRING_ALIGN_CENTER,
    ...
    public void receiveMouseMoveSignal(int x, int y) {
        ...
        float scale = mWaveViewInitScale +
            ((delta * 3.0f) / height * mWaveViewInitScale);
    ...
    private Rectangle getButtonBoundsWaveViewZoom() {
        ...
        return new Rectangle(SPACE, y, width - SPACE - SPACE, height - SPACE -
            y);
    }
}
