...
public class WaveWriter implements IWaveReceiver {
    ...
    public WaveWriter(String path, int channel, int bit_per_sample,
        int sample_rate) throws IOException {
        m_path = path;
    ...
    public void replace(long pos, int length, double[] L, double[] R)
        ...
        long posFile = ((pos * m_channel * m_bit_per_sample) / 8) +
            m_pos_data_chunk;
        ...
        m_total_samples = (m_total_samples < (pos + length)) ? (pos + length)
                                                             : (m_total_samples);
    ...
    public void append(byte[] L, byte[] R) throws IOException {
        ...
        if (m_bit_per_sample == 8) {
            ...
            if (m_channel == 1) {
                for (int i = 0; i < total; i++) {
                    buf = PortUtil.getbytes_int16_le((short) (((L[i] + R[i]) * 128.5f) -
                            32768f));
                    writeByteArray(m_stream, buf, 2);
                }
            } else {
                for (int i = 0; i < total; i++) {
                    buf = PortUtil.getbytes_int16_le((short) ((L[i] * 257f) -
                            32768f));
                    writeByteArray(m_stream, buf, 2);
                    buf = PortUtil.getbytes_int16_le((short) ((R[i] * 257f) -
                            32768f));
                    writeByteArray(m_stream, buf, 2);
                }
            }
