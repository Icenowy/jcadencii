...
public class WaveReader {
    ...
    public WaveReader(String file) throws IOException, FileNotFoundException {
    ...
    public boolean open(String file) throws IOException, FileNotFoundException {
        if (m_opened) {
            m_stream.close();
        }
    ...
    public void read(long start, int length, double[] left, double[] right)
        ...
        int i_start = 0;
        int i_end = length - 1;
        long required_sample_start = start +
            (long) (m_offset_seconds * m_sample_per_sec);
        ...
        if (required_sample_start < 0) {
            ...
            long loc = m_header_offset +
                (m_byte_per_sample * m_channel * required_sample_start);
            m_stream.seek(loc);
        }
    ...
    public void read(long start, int length, ByRef<float[]> left,
        ByRef<float[]> right) throws IOException {
