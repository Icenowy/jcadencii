...
public class MidiEvent implements Comparable<MidiEvent> {
    ...
    public static MidiEvent read(RandomAccessFile stream,
        ByRef<Long> last_clock, ByRef<Integer> last_status_byte)
        ...
        if ((ctrl == 0x80) || (ctrl == 0x90) || (ctrl == 0xA0) ||
                (ctrl == 0xB0) || (ctrl == 0xE0) || (first_byte == 0xF2)) {
            // 3byte使用するチャンネルメッセージ：
            //     0x8*: ノートオフ
            //     0x9*: ノートオン
            //     0xA*: ポリフォニック・キープレッシャ
            ...
            return me;
        } else if ((ctrl == 0xC0) || (ctrl == 0xD0) || (first_byte == 0xF1) ||
                (first_byte == 0xF2)) {
            // 2byte使用するチャンネルメッセージ
            //     0xC*: プログラムチェンジ
            //     0xD*: チャンネルプレッシャ
            ...
            throw new Exception("don't know how to process first_byte: 0x" +
                PortUtil.toHexString(first_byte));
        }
    }
    ...
    public int compareTo(MidiEvent item) {
        if (clock != item.clock) {
            ...
            if (((first_this == 0x80) || (first_this == 0x90)) &&
                    ((first_item == 0x80) || (first_item == 0x90))) {
                if ((data != null) && (data.length >= 2) &&
                        (item.data != null) && (item.data.length >= 2)) {
    ...
    public static MidiEvent generateTimeSigEvent(int clock, int numerator,
        int denominator) {
